...
- **IndexedDB PDF & State Persistence**: Implemented local caching of both the active PDF document (`ArrayBuffer`) and its associated viewer state (current page, zoom level, opacity, pen settings, drawn strokes, and text annotations) in IndexedDB, enabling automatic restoration on page reload.
